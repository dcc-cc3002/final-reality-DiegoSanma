    ...
    def chooseCharacters(turnScheduler: ITurnScheduler): Unit = {
      while (turnScheduler.getPlayers().length < 3) {
        ...
        choice match {
          ...
          case 2 => selected = Some(new Warrior(name, 100, 100, 100))
          case 3 => selected = Some(new Ninja(name, 100, 100, 100))
          case 4 => selected = Some(new WhiteMage(name, 100, 100, 100, 100))
          case 5 => selected = Some(new BlackMage(name, 100, 100, 100, 100))
